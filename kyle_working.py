...
def get_data_file_path():
    ...
    # Use base_path to create a relative path to your data file
    data_file_path = base_path / 'data' / PYTHON
...
def sign_up_page():
    ...
    def switch_to_login():
        # Destroy the current sign-up window or frame
        sign_up.destroy()
        # Call the login window function to display the login page
        login_window()
    ...
    # Open a text file in append mode to store user data
    ...
    def confirm_password_on_hover(e):
        confirm_password_entry.delete(0, "end")

    def confirm_password_on_unhover(e):
        password_value = confirm_password_entry.get()
        if password_value == "":
            confirm_password_entry.insert(0, "Confirm Password")
      
    sign_up = Tk()
    sign_up.geometry("500x500")
    sign_up.title("Sign Up | Chorale")
    sign_up.resizable(False, False)
    sign_up.configure(bg="white")
    
    sign_up_frame=Frame(sign_up, width=350, height=350, bg="white")
    sign_up_frame.place(x=80, y=20)
    
    signup_heading= Label(sign_up_frame, text="Sign Up", fg="skyblue", bg="white", font=("Nue Einstellung", 23))
    signup_heading.place(x=100, y=5)
    

    username_entry = Entry(sign_up_frame, width=25, fg="black", border=0, bg="white", font=("Microsoft YaHei UI Light", 11))
    ...
    password_entry = Entry(sign_up_frame, width=25, fg="black", border=0, bg="white", font=("", 11))
    ...
    confirm_password_entry = Entry(sign_up_frame, width=25, fg="black", border=0, bg="white", font=("", 11))
    confirm_password_entry.place(x=30, y=220)
    confirm_password_entry.insert(0, "Confirm Password")
    Frame(sign_up_frame, width=295, height=2, bg="black").place(x=25, y=245)
    confirm_password_entry.bind("<FocusIn>", confirm_password_on_hover)
    confirm_password_entry.bind("<FocusOut>", confirm_password_on_unhover)

    sign_up_button = Button(sign_up_frame, width=39, pady=7, bg="grey", fg="white", border=0, text="Sign Up", command=register)
    sign_up_button.place(x=33, y=260)

    have_acc = Label(sign_up_frame, text="Already have an existing account?", fg="black", bg="white", font=("Nue Einstellung", 9))
    have_acc.place(x=40, y=320)

    login_btn= Button(sign_up_frame, width=8, text="Log In", border=0, fg="skyblue", bg="white", command=switch_to_login)
    login_btn.place(x=230, y=320)
...
def login_window():
    ...
    def handle_login():
        ...
        with open('users.txt', 'r') as file:
            # Read each line in the file and check if the entered credentials match
            for line in file:
    ...
    def switch_to_sign_up():
        # Destroy the current login window or frame
        login.destroy()
        # Call the sign-up window function to display the sign-up page
        sign_up_page()
...
#Function to resize the image without external libraries
    ...
    signin_heading.place(x=120, y=5)
    ...
    signup_btn= Button(login_frame, width=13, text="Create Account", border=0, fg="skyblue", bg="white", command=switch_to_sign_up)
